...
import { User, Gym, Equipment, Exercise, Challenge, Badge, WorkoutSession } from "../models";
...
export async function runMigrations(services: MigrationServices): Promise<void> {
    try {
        ...
        if (!adminUser?._id) throw new Error('Admin user not found for gym creation');
        ...
        const challenges = await seedChallenges(services.challengeService, adminUser._id.toString(), gyms);
        
        const badges = await seedBadges(services.badgeService, adminUser._id.toString());
        if (badges.length === 0) throw new Error('Aucun badge créé');
        
        const workouts = await seedWorkouts(services.workoutService, users, exercises, gyms, challenges);
